...
private:
	...
	void deleteWithSubstr(TrieTreeNode* root, string ss) {
		queue<string> words;
		...
		TrieTreeNode* token = nullptr;
		while (!words.empty()) {
			string next = words.front();
			words.pop();

			token = searchHelper(root, next);
			if ((token != nullptr) && (next.find(ss) != string::npos) && (token->isTerminal)) {
...
public:
	...
	void RemoveWithSubstr(string ss) {
		deleteWithSubstr(root, ss);
	}
