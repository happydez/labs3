...
class TrieTree {
	...
	private:
		...
		TrieTreeNode* searchHelper(TrieTreeNode* root, string& word) {
			TrieTreeNode* currentNode = root;
		...
		void takeWithHelper(TrieTreeNode* root, string& wseq, queue<string>& words) {
			TrieTreeNode* start = searchHelper(root, wseq);
			...
			for (int i = 0; i < AN; i++) {
				if (start->children[i] != nullptr) {
					inorder(start->children[i], wseq + string(1, 'a' + i), words);
				}
			}
		}
	...
	public:
		...
		queue<string> TakeWith(string word, int n) {
			queue<string> words;
			...
			string wseq = word.substr(0, word.length() - n);
			takeWithHelper(root, wseq, words);
